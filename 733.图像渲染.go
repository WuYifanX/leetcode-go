...
func floodFill(image [][]int, sr int, sc int, newColor int) [][]int {
	...
	isVisited := make([][]bool, len(image))
	for i := 0; i < len(image); i++ {
		isVisited[i] = make([]bool, len(image[i]))
	}
...
func dfs(image *[][]int, x, y, newColor int, isVisited *[][]bool) {
	oldValue := (*image)[x][y]
	(*image)[x][y] = newColor
	(*isVisited)[x][y] = true
	...
	for _, direction := range directions {
		...
		if isValidPosition(newX, newY, len(*image), len((*image)[0])) {
			if (*isVisited)[newX][newY] {
				continue
			}
			if (*image)[newX][newY] == oldValue {
				dfs(image, newX, newY, newColor, isVisited)
			}
		}
	}
}
