...
func countAndSay(n int) string {
	...
	for i := 0; i < len(prevResult); i++ {
		...
		if prevResult[i] == prevResult[i-1] {
			...
			result += fmt.Sprintf("%d%s", currentCount, string(prevResult[i-1]))
			currentCount = 1
		}
	}

	if currentCount != 0 {
		result += fmt.Sprintf("%d%s", currentCount, string(prevResult[len(prevResult)-1]))
	}
	return result
}
